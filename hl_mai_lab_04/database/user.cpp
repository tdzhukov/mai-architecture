...
namespace database
{
    ...
    void User::init()
    {
        try
        {
            ...
            create_stmt << "CREATE TABLE IF NOT EXISTS users (id SERIAL,"
                ...
                now;
            create_stmt << "CREATE UNIQUE INDEX IF NOT EXISTS login_index ON users (login);", now;
            create_stmt << "CREATE INDEX IF NOT EXISTS name_index ON users (first_name, last_name);", now;
        }
    ...
    std::vector<User> User::search(std::string first_name, std::string last_name, std::string login)
    {
        try
        {
            Poco::Data::Session session = database::Database::get().create_session();
            ...
            select << "SELECT id, first_name, last_name, email, title, login, password FROM users where (first_name LIKE $1 and last_name LIKE $2) or login LIKE $3",
                into(a._id),
                into(a._first_name),
                into(a._last_name),
                into(a._email),
