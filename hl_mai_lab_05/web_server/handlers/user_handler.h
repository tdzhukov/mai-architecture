        ...
        try
        {
            if (form.has("id") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET))
            {
                ...
                if (!result) {
                    ...
                    if (result) result->save_to_cache();
                }
                if (result)
                {
            ...
            else if (hasSubstr(request.getURI(), "/search"))
            {
                ...
                std::string lgn = form.get("login", "");
                if ((fn.empty() || ln.empty()) && lgn.empty()) {
                    response.setStatus(Poco::Net::HTTPResponse::HTTP_NOT_FOUND);
                    std::ostream &ostr = response.send();
                    ostr << "Missing required fields";
                    response.send();
                    return;
                }

                auto results = database::User::search(fn, ln, lgn);
                Poco::JSON::Array arr;
                for (auto s : results)
            ...
            else if (request.getMethod() == Poco::Net::HTTPRequest::HTTP_POST)
            {
                if (form.has("first_name") && form.has("last_name") && form.has("email") && form.has("title") && form.has("login") && form.has("password"))
                {
                    ...
                    std::cout << form.has("first_name") << std::endl;
                    response.setStatus(Poco::Net::HTTPResponse::HTTP_NOT_FOUND);
                    std::ostream &ostr = response.send();
                    ostr << "Missing required fields";
                    response.send();
